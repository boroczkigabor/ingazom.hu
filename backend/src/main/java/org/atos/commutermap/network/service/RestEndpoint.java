...
public class RestEndpoint implements org.atos.commutermap.network.service.BaseStationsApi, org.atos.commutermap.network.service.DestinationsForMapApi {
    ...
    public Iterable<Destination> destinationsForGoogleMap(@PathVariable("departureStation") String departureStation) {
        ...
        return StreamSupport.stream(routeRepository.getAllRoutesByDepartureStation(departureStat.get())
                ...
                .map(route ->
                        new Destination()
                                .departure(Optional.ofNullable(route.getRealDepartureStation()).orElse(route.getDepartureStation()).name)
                                .destination(route.getDestinationStation().name)
                                .lat(BigDecimal.valueOf(route.getDestinationStation().coordinates.latitude))
                                .lon(BigDecimal.valueOf(route.getDestinationStation().coordinates.longitude))
                                .minutes(String.valueOf(route.getDuration().toMinutes()))
